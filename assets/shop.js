...
/* The hero buy box offers 3 sizes of the same window hanging, switched with a
   pill selector — each size is its own catalogue product id (see MOCK_PRODUCTS
   and server.js CATALOGUE), not a separate variant system. */
const SIZES = [
  { id:'malai-1ft', label:'1 ft' },
  { id:'malai-2ft', label:'2 ft' },
  { id:'malai-3ft', label:'3 ft' },
];
...
/* Static preview data — used only when /api is unreachable (e.g. this page
   ...
   The live site always prefers the real API; this never overrides it.
   PLACEHOLDER PRICING for 2ft/3ft/long — see note in server.js CATALOGUE. */
const MOCK_PRODUCTS = [
  { id:'malai-1ft',         name:'Window Hanging (Malai) — 1 ft', tagline:'Hand-tied vetiver root mala',   grade:'Finest long fibre', price:899,  mrp:1299, img:'assets/garland-1ft.jpg', available:42, total:50, status:'in' },
  { id:'malai-2ft',         name:'Window Hanging (Malai) — 2 ft', tagline:'Hand-tied vetiver root mala',   grade:'Finest long fibre', price:1449, mrp:1999, img:'assets/garland-2ft.jpg', available:33, total:40, status:'in' },
  { id:'malai-3ft',         name:'Window Hanging (Malai) — 3 ft', tagline:'Hand-tied vetiver root mala',   grade:'Finest long fibre', price:1999, mrp:2699, img:'assets/garland-3ft.jpg', available:21, total:30, status:'in' },
  // No distinct real photo exists yet for this product — reuses the malai artwork as a placeholder.
  { id:'window-garland-long', name:'Long Window Garland',         tagline:'The long-strand window piece',  grade:'Finest long fibre', price:1699, mrp:2299, img:'assets/garland-long.jpg', available:24, total:30, status:'in' },
  { id:'cushion-classic',   name:'Cushion',                       tagline:'Woven root, cool to touch',     grade:'Medium fibre',      price:1249, mrp:1699, img:'assets/p-mat.jpg',          available:25, total:25, status:'in' },
  { id:'quils-classic',     name:'Quils',                         tagline:'Hand-rolled root quils',        grade:'Short fibre',       price:399,  mrp:599,  img:'assets/p-pad.jpg',          available:80, total:80, status:'in' },
];
...
async function loadCatalogue(){
  ...
  // The 3 malai sizes already have their own buy box above (size selector); don't repeat them here.
  ...
  el.querySelectorAll('[data-add]').forEach(b => {
    b.onclick = () => {
      ...
      addItem({ id:p.id, name:p.name, price:p.price, img:p.img }, 1, p.available);
    };
  });
}
...
function renderPrice(){
  ...
  $('heroImg').src = p.img;
  $('heroImg').alt = `Vetriver ${p.name} — handwoven vetiver root mala`;
}

if($('sizePills')){
  $('sizePills').querySelectorAll('button').forEach(b => {
    b.onclick = () => {
...
// keep Tab inside the open cart dialog
$('drawer').addEventListener('keydown', e => {
  if(e.key !== 'Tab') return;
  const items = [...$('drawer').querySelectorAll('a[href],button:not([disabled]),input,select,textarea')]
...
if($('addBtn'))    $('addBtn').onclick    = () => addItem(heroCartItem(), qty, stock.available ?? 10);
if($('buyNowBtn')) $('buyNowBtn').onclick = () => { addItem(heroCartItem(), qty, stock.available ?? 10); openCart(); };
